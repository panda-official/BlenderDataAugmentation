...
def camera_view_bounds_2d(
    scene, cam_ob, me_ob
):  # Returns bb coordinates of an object in camera space
    """
...
def get_rotation_matrix(obj):
    ...
    for k in range(3):
        matrix_string += (
            str(rotation_matrix[0][k])
            + " "
            + str(rotation_matrix[1][k])
            + " "
            + str(rotation_matrix[2][k])
            + " "
        )
...
def generate_bb_and_render(
    ...
    if task == "preview":
        ...
        scene.render.resolution_x = (
            scene.render.resolution_x * scene.data_generation.res_scale
        )
        scene.render.resolution_y = (
            scene.render.resolution_y * scene.data_generation.res_scale
        )
    ...
    for i in range(1, number_of_frames + 1):
        ...
        print(
            f"Frames {i} of {number_of_frames} rendered, to cancell close blender window"
        )
...
def rand_transform(
    x, y, z, task
):  # Returns np.array to use as vector randomized in given ranges
    if task == "loc":
        transform = np.array(
            [
                random.uniform(x / -2, x / 2),
                random.uniform(y / -2, y / 2),
                random.uniform(z / -2, z / 2),
            ]
        )
    elif task == "scale":
        transform = np.array(
            [
                random.uniform(x / -2 + 1, x / 2 + 1),
                random.uniform(y / -2 + 1, y / 2 + 1),
                random.uniform(z / -2 + 1, z / 2 + 1),
            ]
        )
    elif task == "rot":
        x = x * math.pi / 180
    ...
    if task == "generate":
        ...
        for obj in objs:
            ...
            for i in range(1, number_of_frames):
                obj.rotation_euler = rand_transform(
                    scene.data_generation.rotation_variation_X,
                    scene.data_generation.rotation_variation_Y,
                    scene.data_generation.rotation_variation_Z,
                    "rot",
                )
                obj.keyframe_insert(data_path="rotation_euler", frame=i)
                obj.location = object_initial_location[:] + rand_transform(
                    scene.data_generation.translation_variation_X,
                    scene.data_generation.translation_variation_Y,
                    scene.data_generation.translation_variation_Z,
                    "loc",
                )
                obj.keyframe_insert(data_path="location", frame=i)
                obj.scale = rand_transform(
                    scene.data_generation.scale_variation_X,
                    scene.data_generation.scale_variation_Y,
                    scene.data_generation.scale_variation_Z,
                    "scale",
                )
        ...
        augment_dict = {
            f"{class_to_aug_str}": {
                "translation variations": str(
                    scene.data_generation.translation_variation_X
                )
                + " "
                + str(scene.data_generation.translation_variation_Y)
                + " "
                + str(scene.data_generation.translation_variation_Z),
                "rotation variations": str(scene.data_generation.rotation_variation_X)
                + " "
                + str(scene.data_generation.rotation_variation_Y)
                + " "
                + str(scene.data_generation.rotation_variation_Z),
                "scale variations": str(scene.data_generation.scale_variation_X)
                + " "
                + str(scene.data_generation.scale_variation_Y)
                + " "
                + str(scene.data_generation.scale_variation_Z),
...
def augment_enviro(scene, number_of_frames, task):
    ...
    plane_strenght = (
        bpy.data.materials["PLANE_MAT"].node_tree.nodes["Emission"].inputs[1]
    )
    spot_strenght = bpy.data.lights["Spot"].node_tree.nodes["Emission"].inputs[1]
    plane_temperature = (
        bpy.data.materials["PLANE_MAT"].node_tree.nodes["Blackbody"].inputs[0]
    )
    ...
    if task == "augment":
        for i in range(1, number_of_frames):
            ...
            # empty augmentation
            empty.location = empty_init_loc[:] + rand_transform(
                0, 0, scene.data_generation.empty_z_variation / 5, "loc"
            )
            empty.keyframe_insert(data_path="location", frame=i)
            empty.rotation_euler = rand_transform(
                0, 0, scene.data_generation.empty_z_variation_rot, "rot"
            )
            empty.keyframe_insert(data_path="rotation_euler", frame=i)

            # lamps jitter
            plane.location = plane_init_loc[:] + rand_transform(
                scene.data_generation.lamps_jitter / 10,
                scene.data_generation.lamps_jitter / 10,
                scene.data_generation.lamps_jitter / 10,
                "loc",
            )
            plane.keyframe_insert(data_path="location", frame=i)
            spot.location = spot_init_loc[:] + rand_transform(
                scene.data_generation.lamps_jitter / 10,
                scene.data_generation.lamps_jitter / 10,
                scene.data_generation.lamps_jitter / 10,
                "loc",
            ...
            plane_strenght.default_value = (
                init_plane_strenght
                - init_plane_strenght
                * random.uniform(
                    scene.data_generation.lamps_strength / -2,
                    scene.data_generation.lamps_strength / 2,
                )
            )
            plane_strenght.keyframe_insert(data_path="default_value", frame=i)
            spot_strenght.default_value = (
                init_spot_strenght
                - init_spot_strenght
                * random.uniform(
                    scene.data_generation.lamps_strength / -2,
            ...
            plane_temperature.default_value = (
                init_plane_temperature
                - init_plane_temperature
                * random.uniform(
                    scene.data_generation.lamps_temperature / -2,
                    scene.data_generation.lamps_temperature / 2,
                )
            )
            plane_temperature.keyframe_insert(data_path="default_value", frame=i)
            spot_temperature.default_value = (
                init_spot_temperature
                - init_spot_temperature
                * random.uniform(
                    scene.data_generation.lamps_temperature / -2,
                    scene.data_generation.lamps_temperature / 2,
...
def set_enviro(dict):  # Used for augumenting the envirorment from dictionary
    ...
    scene.data_generation.empty_z_variation_rot = float(
        dict.get("rotation variation empty")
    )
    scene.data_generation.lamps_jitter = float(dict.get("jitter"))
    scene.data_generation.number_of_frames_aug_enviro = int(
        dict.get("augmented frames envirorment")
    )
...
def set_class(dict, name):  # Used for augmenting objects from dictionary
    ...
    scene.data_generation.translation_variation_X = split_three(
        dict.get("translation variations"), "x"
    )
    scene.data_generation.translation_variation_Y = split_three(
        dict.get("translation variations"), "y"
    )
    scene.data_generation.translation_variation_Z = split_three(
        dict.get("translation variations"), "z"
    )
    scene.data_generation.rotation_variation_X = split_three(
        dict.get("rotation variations"), "x"
    )
    scene.data_generation.rotation_variation_Y = split_three(
        dict.get("rotation variations"), "y"
    )
    scene.data_generation.rotation_variation_Z = split_three(
        dict.get("rotation variations"), "z"
    )
    scene.data_generation.scale_variation_X = split_three(
        dict.get("scale variations"), "x"
    )
    scene.data_generation.scale_variation_Y = split_three(
        dict.get("scale variations"), "y"
    )
    scene.data_generation.scale_variation_Z = split_three(
        dict.get("scale variations"), "z"
    )
    scene.data_generation.number_of_frames_aug = dict.get("augmented frames")

    class_to_augment = bpy.data.collections[f"{name}"]
    augment(
        "generate", scene, scene.data_generation.number_of_frames_aug, class_to_augment
    )
